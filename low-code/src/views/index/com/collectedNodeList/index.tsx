...
const CollectNodeList: FC<iprops> = ({}, _ref): ReactElement => {
	//===============useHooks=================
			...
			<WindowLay
				title={
					<div className={styles.header}>
						...
						<span>{rendererData.mouseMode === "componentEdit" ? "收藏夹 - 用户控件列表" : "收藏夹 - 布局节点列表"}</span>
					</div>
				}
				isShow={rendererData.isopenCollectedBucket && rendererData.mouseAction === "free" && rendererData.mouseMode !== "none"}
				onClose={function () {
					rendererData.setisopenCollectedBucket(false);
				}}
							...
							return (
								<>
									<div className={styles.cardContainer}>
										{(JSON.parse(rendererData.collectedNodes) as unknown as IcollectingItem[]).map(function (item, index) {
											if (item.node.nodetype === "layout") {
												return (
													<React.Fragment key={item.node.name + "_" + index}>
						...
						if (rendererData.mouseMode === "componentEdit") {
							return (
								<>
									<div className={styles.cardContainer}>
										{(JSON.parse(rendererData.collectedNodes) as unknown as IcollectingItem[]).map(function (item, index) {
											if (item.node.nodetype === "component") {
												return (
													<React.Fragment key={item.node.name + "_" + index}>
...
export default CollectNodeList;
